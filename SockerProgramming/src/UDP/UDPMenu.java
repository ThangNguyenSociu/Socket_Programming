...
public class UDPMenu extends JFrame {
	...
	private String getLocalPhysicalIP() {
		try {
			...
			while (interfaces.hasMoreElements()) {
				...
				// LOẠI BỎ CÁC MẠNG ẢO & VPN (Cập nhật: Radmin, Hamachi, ZeroTier)
				if (!dN.contains("vmware") && !dN.contains("virtual") && !dN.contains("vbox")
						&& !dN.contains("radmin") && !dN.contains("hamachi") && !dN.contains("zerotier")
						&& !dN.contains("tunnel") && !dN.contains("teredo") && !dN.contains("pseudo")) {
	...
	private JButton createNavButton(String title, String subtitle) {
		JButton btn = new JButton(
				"<html><div style='padding-left: 10px; width: 220px;'><b style='font-size: 13px; color: #2c3e50;'>"
						+ title + "</b><br><i style='font-size: 9px; color: #95a5a6;'>" + subtitle
						+ "</i></div></html>");
		btn.setFont(navFont);
		...
		btn.addMouseListener(new java.awt.event.MouseAdapter() {
			public void mouseEntered(java.awt.event.MouseEvent e) {
				...
				btn.setBorder(BorderFactory.createLineBorder(primaryColor));
			}

			public void mouseExited(java.awt.event.MouseEvent e) {
				btn.setBackground(Color.WHITE);
				btn.setBorder(BorderFactory.createLineBorder(new Color(238, 238, 238)));
			}
		});
		return btn;
	}
	...
	private JPanel createWelcomeScreen() {
		...
		JLabel l = new JLabel(
				"<html><center><h1 style='color: #2c3e50; font-family: Segoe UI; font-size: 24px;'>Hệ thống Thực nghiệm Socket</h1>"
						+
